...
* `test-htb.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `test-htb-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `test-htb-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).

## <a name='gettingStarted'></a>Getting Started
1. <b>Complete the test-htb.js file</b>
    * test-htb.js is where all of your adapter code will live.
...
2. <b>Complete the test-htb-validator.js file</b>
...
3. <b>Complete the test-htb-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.TestHtb` will be accessible through `window.headertag.TestHtb`
...
In this section you will be filling out the test-htb.js, test-htb-exports.js, and the test-htb-validator.js files to create your module.

### Step 0: Config Validation (`test-htb-validator.js`)
...
### Step 2: Generate Request URL (`test-htb.js`)
...
### Step 3: Response Callback (`test-htb.js`)
...
### Step 4: Parsing and Storing Demand (`test-htb.js`)
...
### Step 5: Rendering Pixel (`test-htb.js`)
...
### Step 6: Exports (`test-htb-exports.js`)
...
```javascript
shellInterface.TestHtb = { //shell interface is the window variable that is accessible through the window object, currently this will always be window.headertag
    render: SpaceCamp.services.RenderService.renderDfpAd.bind(null, 'TestHtb')
};
```
...
```javascript
if (__directInterface.Layers.PartnersLayer.Partners.TestHtb) {
    shellInterface.TestHtb = shellInterface.TestHtb || {};
    shellInterface.TestHtb.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.TestHtb.adResponseCallback;
}
```
...
if (__directInterface.Layers.PartnersLayer.Partners.TestHtb) {
    shellInterface.TestHtb = shellInterface.TestHtb || {};
    shellInterface.TestHtb.adResponseCallbacks = __directInterface.Layers.PartnersLayer.Partners.TestHtb.adResponseCallbacks;
}
```
